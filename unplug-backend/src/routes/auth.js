...
router.post('/login', loginLimiter, async (req, res, next) => {
  try {
    ...
    const result = await pool.query(
      'SELECT id, email, role, password_hash, email_verified, full_name, member_type, is_suspended, suspended_reason, two_factor_enabled, free_publishing_enabled, is_representative FROM users WHERE email = $1',
      [email]
    );
    const user = result.rows[0];
    ...
    const token = jwt.sign(
      { id: user.id, email: user.email, role: user.role, free_publishing_enabled: user.free_publishing_enabled, is_representative: user.is_representative },
      process.env.JWT_SECRET,
      { expiresIn: '7d' }
    );

    res.json({
      user: { id: user.id, email: user.email, role: user.role, full_name: user.full_name, member_type: user.member_type, is_representative: user.is_representative },
      token,
    });
  } catch (err) {
    next(err);
...
router.post('/magic-link/consume', async (req, res, next) => {
  try {
    ...
    const userResult = await pool.query(
      'SELECT id, email, role, is_suspended, suspended_reason, free_publishing_enabled, is_representative FROM users WHERE id = $1',
    ...
    res.json({ user: { id: user.id, email: user.email, role: user.role, is_representative: user.is_representative }, token: authToken });
  } catch (err) {
    next(err);
  }
});
