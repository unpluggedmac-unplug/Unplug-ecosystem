  ...
  try {
   const { category, package: packageTier, type, ids } = req.query;
    ...
    const result = await pool.query(
      `SELECT p.id, p.slug, p.display_name, p.package_tier, p.bio, p.type, p.deaf_owned_verified, p.feature_image_url,
              p.cover_animation_effect, p.cover_transition_duration_ms, c.name AS category, c2.name AS secondary_category
       FROM profiles p
       LEFT JOIN categories c ON c.id = p.category_id
       LEFT JOIN categories c2 ON c2.id = p.secondary_category_id
       WHERE ${conditions.join(' AND ')}
