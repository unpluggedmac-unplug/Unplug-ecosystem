...
const {
  uploadGrowthFile,
  verifySignature,
  ALLOWED_GROWTH_FILE_MIME_TYPES,
  MAX_GROWTH_FILE_SIZE_BYTES,
...
// Growth Application media and documents can contain personal/confidential
// material. They are stored in the PRIVATE R2 bucket and catalogued against
// the owning application. Production/staging fail closed if private R2 is not
// configured; there is no Render-local fallback.
router.post('/', requireAuth, requireMember, (req, res) => {
  ...
  uploadGrowthFile.single('file')(req, res, async (err) => {
    if (err) {
      const message = err.code === 'LIMIT_FILE_SIZE'
        ? `That file is too large. The maximum is ${Math.round(MAX_GROWTH_FILE_SIZE_BYTES / (1024 * 1024))}MB.`
        : err.message;
    ...
    const signature = verifySignature(req.file, ALLOWED_GROWTH_FILE_MIME_TYPES);
    if (!signature.ok) {
    ...
    try {
      ...
      if (fieldKey) {
        const fieldResult = await pool.query(
          `SELECT field_key, field_type, audience, confidential, allow_external_sharing,
                  sensitive, sensitive_enabled
        ...
        const uploadTypes = new Set(['upload','image_upload','document_upload','portfolio_upload']);
        if (!fieldResult.rowCount
            || !uploadTypes.has(fieldResult.rows[0].field_type)
            || (fieldResult.rows[0].sensitive && !fieldResult.rows[0].sensitive_enabled)) {
          fs.unlink(req.file.path, () => {});
          return res.status(400).json({ error: 'That upload field is not enabled for this application.' });
        }
        field = fieldResult.rows[0];
        if (field.field_type === 'image_upload' && req.file.mimetype === 'application/pdf') {
          fs.unlink(req.file.path, () => {});
          return res.status(400).json({ error: 'This field accepts an image, not a PDF.' });
        }
      }
      ...
      const objectKey = await uploads.uploadPrivateBuffer(
        buffer,
      ...
      const saved = await pool.query(
        ...
        [applicationId, fieldKey, objectKey, req.file.originalname || req.file.filename,
