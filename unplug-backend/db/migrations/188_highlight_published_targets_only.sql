...
-- Content flow. The browser already only offers approved items, but a caller
-- must not be able to bypass that rule by posting an unpublished article/profile
-- id directly to the API.
--
...
BEGIN
  ...
  IF target_status <> 'approved' THEN
    RAISE EXCEPTION 'Only published content can be highlighted.'
      USING ERRCODE = '23514';
  END IF;
